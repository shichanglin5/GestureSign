    ...
    public class PreviousApplication : IPlugin
    {
        ...
        public bool Gestured(PointInfo ActionPoint)
        {
            try
            {
                ...
                // If current window is not in the list, start from end
                if (currentIndex == -1)
                {
                    Logging.LogDebug("[PreviousApplication] Current window not in switchable list, starting from last window");
                    currentIndex = windows.Count; // Will become Count-1 after -1
                }

                // Get previous window (cycle to last if at beginning)
                int previousIndex = (currentIndex - 1 + windows.Count) % windows.Count;
                IntPtr previousWindow = windows[previousIndex];
            ...
            {
                Logging.LogError($"[PreviousApplication] Error: {ex.Message}");
                return false;
            }
        }
